...
async def eml_vv_test(dut):
    ...
    # Tolerance analysis from EMLUnit.sv LUT approximation structure:
    # ExpApprox: 128-segment piecewise-linear interpolator (7-bit address,
    #   `addrFull` at EMLUnit.sv:362), 8 pipeline stages. Each segment uses
    #   linear slope+offset from lookup ROM. For FP32 with 23-bit mantissa,
    #   piecewise-linear with 128 segments yields ~1-2 ULP max error.
    # LnApprox: similar piecewise-linear with 4 pipeline stages + 4 delay
    #   registers (EMLUnit.sv:654). ~1-2 ULP max error.
    # FP32Adder subtract: final FP32 subtraction adds ~0.5 ULP round error.
    # Combined worst-case: ~3-4 ULP at magnitude ~100.
    # 1 ULP at |x|=100: 100 * 2^-23 ≈ 1.19e-5.
    # 4 ULP: ~4.8e-5 absolute. Using atol=5e-3 provides ~100x safety margin
    # over the analytically derived bound.
    # Reference: EMLUnit.sv modules ExpApprox (line 150), LnApprox (line 654),
    # FP32Adder (line 37), FP32Multiplier (line 2) — all generated by
    # CIRCT firtool-1.62.1 piecewise-linear floating-point lowering.
    atol = 5e-3
